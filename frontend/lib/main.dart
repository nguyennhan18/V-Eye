...
import 'package:camera/camera.dart'; // Đã thêm thư viện camera
import 'dart:math';

// Khai báo biến toàn cục lưu danh sách camera
List<CameraDescription> cameras = [];

Future<void> main() async {
  // Đảm bảo Flutter khởi tạo xong trước khi tìm camera
...
class _HomeScreenState extends State<HomeScreen>
  ...
  Future<void> _initializeCamera() async {
    ...
    // Chọn camera sau (thường là index 0)
    _cameraController = CameraController(
      cameras[0],
      ResolutionPreset.high,
      enableAudio: false, // Tắt audio để không xung đột thu âm
    );
  ...
  void dispose() {
    ...
    _cameraController?.dispose(); // Tắt camera khi đóng app
    super.dispose();
  }
  ...
  Future<void> _onDoubleTap() async {
    ...
    if (!_isListening) {
      ...
      // BƯỚC 1: ĐANG NGHE (Mô phỏng 2 giây)
      ...
      // BƯỚC 2: CHỤP ẢNH NGẦM & PHÂN TÍCH (Lúc UI đang hiện "AI đang phân tích...")
      if (_isCameraReady && _cameraController != null) {
        try {
          print("📸 Đang thực hiện chụp ảnh ngầm...");
          XFile file = await _cameraController!.takePicture();
          
          print("✅ Đã chụp xong! Ảnh được lưu tại: ${file.path}");
          // Tương lai: Gọi API Gemini/Vision tại đây và truyền file.path lên
      ...
      // Giả lập thời gian chờ AI trả kết quả (2 giây)
      await Future.delayed(const Duration(seconds: 2));
      if (!mounted) return;
